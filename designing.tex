...
\begin{itemize}
\item a \ct{LaserScan} entity with a \ct{ranges} source providing laser
...
\item a light projector entity that can be activated or deactivated on
  request;
\item a \ct{Camera} and \ct{Light} entities that respectively take
...
\ct{ObstacleDetection} context operator.
...
information from the \ct{ObstacleZone} context operator to trigger the
...
decide to reuse the \ct{Twsit} data type as provided by ROS. The robot
current mode is represented as an enumeration of the \ct{RANDOM} and
...
decided to reuse the \ct{Twist} data type of the ROS middleware which
is illustrated by the \ct{import} line.
...
The \ct{interaction} keyword is used to introduce an interaction
